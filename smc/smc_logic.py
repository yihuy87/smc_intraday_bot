...
# ============================================================
#               LOGIC SMC INTRADAY APEX
# ============================================================

def detect_bias_generic(df: pd.DataFrame) -> bool:
    """
    Bias generik intraday (Apex):
    - close > EMA20 > EMA50
    - EMA20 & EMA50 naik dibanding ~8 candle lalu (slope positif)
    """
    close = df["close"]
    ema20 = ema(close, 20)
    ema50 = ema(close, 50)
...
def detect_momentum(df_5m: pd.DataFrame):
    """
    Momentum intraday Apex:
    - OK: RSI 45–70
    - Premium: RSI 54–60 (zona "emas" continuation intraday)
    """
    closes = df_5m["close"]
    if len(closes) < 30:
        # Data terlalu pendek, jangan terlalu strict, tapi bukan premium
        return True, False

    rsi_val = rsi(closes, 14).iloc[-1]

    momentum_ok = bool(45 < rsi_val < 70)
    momentum_premium = bool(54 < rsi_val < 60)
...
def detect_not_choppy(df_5m: pd.DataFrame, window: int = 25) -> bool:
    ...
    if len(highs) < window + 5:
        return False  # butuh histori cukup untuk Apex
    ...
    # Dominasi arah close (lebih dari 60% bar naik atau turun)
...
def detect_not_overextended(df_5m: pd.DataFrame,
                            ema_period: int = 20,
                            max_distance_pct: float = 0.01) -> bool:
    """
    TRUE kalau harga TIDAK terlalu jauh dari EMA (tidak over-extended).
    Apex: jarak ke EMA20 lebih ketat (default 1%).
    Untuk long:
...
def detect_volatility_regime(df_5m: pd.DataFrame,
    ...
    if len(atr_series) < lookback:
        # tidak cukup data, jangan terlalu strict
        return True
    ...
    # True kalau ATR di tengah-tengah distribusi (bukan extreme low/high)
...
def analyse_symbol(symbol: str):
    """
    Versi SMC INTRADAY APEX (LONG only):
    - Timeframe entry: 5m
    - Konfirmasi trend: 15m
    - Trend besar: 1H
    - Bias 5m/15m/1H: close > EMA20 > EMA50 (+ ema naik)
    - Trigger: micro CHoCH premium + micro FVG (wajib)
    - Filter: momentum premium, tidak choppy, tidak overextended,
    ...
    # Syarat inti Apex:
    # - Bias semua TF OK
    # - Trend strength cukup kuat
    # - Wajib micro CHoCH premium
    # - Wajib micro FVG
    # - Momentum OK + premium
    # - Market tidak choppy
    # - Tidak over-extended
    ...
    # Hindari entry di pucuk candle terakhir (Apex lebih ketat sedikit)
    ...
    conditions = {
        ...
        # tambahan info Apex (optional, tidak dipakai scoring sekarang)
