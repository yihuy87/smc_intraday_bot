...
def build_signal_message(
    ...
    text = f"""🟦 SMC INTRADAY APEX — {symbol}

TF: Entry 5m | Trend 15m & 1H
...
• Momentum OK (zona 45–70)        : {mark(momentum_ok)}
• Momentum premium (zona 54–60)   : {mark(momentum_premium)}
• Market tidak choppy             : {mark(not_choppy)}
• Tidak over-extended dari EMA    : {mark(not_overextended)}

📝 Catatan
• Strategi intraday kualitas tinggi (Apex).
• Entry di 5m, searah bias 15m dan 1H.
• Fokus continuation trend dengan confluence kuat.
• Tier A+ = confluence / kualitas paling tinggi.
